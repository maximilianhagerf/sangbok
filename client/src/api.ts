...
export const createSong = (body: Partial<Song>) => request<Song>('/api/songs', { method: 'POST', body: JSON.stringify(body) })
export const updateSong = (id: number, body: Partial<Song>) => request<Song>(`/api/songs/${id}`, { method: 'PUT', body: JSON.stringify(body) })
export const deleteSong = (id: number) => request<void>(`/api/songs/${id}`, { method: 'DELETE' })
export const reorderSongs = (ids: number[]) => request<void>('/api/songs/reorder', { method: 'PATCH', body: JSON.stringify({ ids }) })
...
export const deleteSection = (id: number) => request<void>(`/api/sections/${id}`, { method: 'DELETE' })
export const reorderSections = (songId: number, ids: number[]) =>
  request<void>(`/api/songs/${songId}/sections/reorder`, { method: 'PATCH', body: JSON.stringify({ ids }) })
