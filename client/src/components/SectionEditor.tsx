...
export default function SectionEditor({ section, onUpdate, onDelete }: Props) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="border border-stone-200 rounded-lg bg-white overflow-hidden">
      {/* Header bar */}
      ...
      <div className="border-t border-stone-100">
        ...
        >
          ...
          Chords {chords.length > 0 && <span className="ml-1 text-amber-600">({chords.length})</span>}
        </button>

        {chordsOpen && (
          <div className="px-4 pb-3 space-y-1.5">
            <div className="grid grid-cols-[1.5rem_1fr_2fr_1.5rem] gap-2 text-xs text-stone-500 font-medium mb-1">
              <span>#</span><span>Line preview</span><span>Chord</span><span></span>
            </div>
            {chords.map(({ line, chord }) => (
              <div key={line} className="grid grid-cols-[1.5rem_1fr_2fr_1.5rem] gap-2 items-center">
                ...
                />
                <button onClick={() => removeChord(line)} className="text-stone-400 hover:text-rose-500">
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
