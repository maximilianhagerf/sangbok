...
router.get('/:id', (req, res) => {
  ...
  const sections = db.prepare(`SELECT * FROM sections WHERE song_id = ? ORDER BY position`).all(req.params.id) as any[]
  res.json({ ...song, sections: sections.map(s => ({ ...s, chords: s.chords ? JSON.parse(s.chords) : [] })) })
})
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE songs SET title = COALESCE(?, title), credit = ?, original = ?, columns = COALESCE(?, columns) WHERE id = ?`)
...
router.post('/:songId/sections', (req, res) => {
  ...
  const maxPos = (db.prepare(`SELECT MAX(position) as m FROM sections WHERE song_id = ?`).get(req.params.songId) as any).m ?? 0
  const result = db.prepare(
    `INSERT INTO sections (song_id, label, content, chords, position) VALUES (?, ?, ?, ?, ?)`
  ).run(req.params.songId, label ?? null, content, chords ? JSON.stringify(chords) : null, maxPos + 1)

  const section = db.prepare(`SELECT * FROM sections WHERE id = ?`).get(result.lastInsertRowid) as any
...
router.patch('/:songId/sections/reorder', (req, res) => {
  ...
  const reorder = db.transaction((orderedIds: number[]) => {
    orderedIds.forEach((id, idx) => update.run(idx + 1, id, req.params.songId))
  })
  reorder(ids)
  res.status(204).end()
})
