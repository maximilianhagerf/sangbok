...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE sections SET label = ?, content = COALESCE(?, content), chords = ? WHERE id = ?`)
    .run(label ?? null, content ?? null, chords !== undefined ? JSON.stringify(chords) : null, req.params.id)
